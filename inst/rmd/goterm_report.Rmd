...
rmd_params$verbose <- params$verbose

#' Only print text if verbose
#' @param dat Object to print if verbose is enabled 
print_verbose <- function(dat) {
  if (rmd_params$verbose) {
    print(dat)
  }
}
...
tibble::tibble(
  name = c("output_path", "save_excel", "significance_cutoff", "simplify_ontologies", "do_gse", "use_background", "store_r_objects", "simplify_cutoff", "save_plots_as_pdf", "verbose"),
  setting = c(rmd_params$output_path, rmd_params$save_excel, rmd_params$significance_cutoff, rmd_params$simplify_ontologies, rmd_params$do_gse, rmd_params$use_background, rmd_params$store_r_objects, rmd_params$simplify_cutoff, rmd_params$save_plots_as_pdf, rmd_params$verbose)
) %>%
  knitr::kable()
...
if (rmd_params$store_r_objects) {
  # Verbose output
  paste0("Export R objects to ", file.path(rmd_params$output_path, "dat_goterms.rds")) %>%
    print_verbose()
...
if (rmd_params$save_excel) {
  ...
  paste0("Export excel sheet to ", export_path) %>% print_verbose()
