...
tibble::tibble(
  name = c("output_path", "save_excel", "significance_cutoff", "simplify_ontologies", "do_gse", "use_background", "store_r_objects", "simplify_cutoff", "save_plots_as_pdf"),
  setting = c(rmd_params$output_path, rmd_params$save_excel, rmd_params$significance_cutoff, rmd_params$simplify_ontologies, rmd_params$do_gse, rmd_params$use_background, rmd_params$store_r_objects, rmd_params$simplify_cutoff, rmd_params$save_plots_as_pdf)
) %>%
  knitr::kable()
...
print_goterm_as_datatable <- function(dat) {
  dat %>%
    ...
    dplyr::select(-geneID, -pvalue, -qvalue) %>%
    DT::datatable(
      extensions = 'Buttons',
      options = list(
        dom = 'Bfrtip',
...
The Percentage Overlap Plot plots the top n GO-terms w.r.t. overall overlap percentage in the GO-term.
The adjusted p-value is mapped to color and the number of significant genes to circle size.

```{r bp_overlap_percentage_plot, fig.width = 12, fig.height = 5}
...
dat_goterms$Molecular_Function %>%
  print_goterm_as_datatable()
```
```{r mf_overlap_percentage_plot, fig.width = 12, fig.height = 5}
dat_goterms$Molecular_Function %>%
  mygo::attach_goterm_genecount() %>%
  mygo::overlap_percentage_plot()
```
...
dat_goterms$Cellular_Components %>%
  print_goterm_as_datatable()
```
```{r cc_overlap_percentage_plot, fig.width = 12, fig.height = 5}
dat_goterms$Cellular_Components %>%
  mygo::attach_goterm_genecount() %>%
  mygo::overlap_percentage_plot()
```
...
```
```{r bp_simple_overlap_percentage_plot, fig.width = 12, fig.height = 5}
if (rmd_params$simplify_ontologies) {
  dat_goterms_simplified$Biological_Process %>%
    mygo::attach_goterm_genecount() %>%
    mygo::overlap_percentage_plot()
...
```
```{r mf_simple_overlap_percentage_plot, fig.width = 12, fig.height = 5}
if (rmd_params$simplify_ontologies) {
  dat_goterms_simplified$Molecular_Function %>%
    mygo::attach_goterm_genecount() %>%
    mygo::overlap_percentage_plot()
...
```
```{r cc_simple_overlap_percentage_plot, fig.width = 12, fig.height = 5}
if (rmd_params$simplify_ontologies) {
  dat_goterms_simplified$Cellular_Components %>%
    mygo::attach_goterm_genecount() %>%
    mygo::overlap_percentage_plot()
