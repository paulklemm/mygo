...
tibble::tibble(
  name = c("output_path", "save_excel", "significance_cutoff", "simplify_ontologies", "do_gse", "use_background"),
  setting = c(rmd_params$output_path, rmd_params$save_excel, rmd_params$significance_cutoff, rmd_params$simplify_ontologies, rmd_params$do_gse, rmd_params$use_background)
) %>%
  knitr::kable()
...
if (!('EntrezID' %in% colnames(rmd_params$dat))) {
  ...
  # Print empty string because otherweise the status message will be rendered distorted
  print("")
